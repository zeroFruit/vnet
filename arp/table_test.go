...
func TestTable(t *testing.T) {
	...
	entry1, ok := table.Lookup(arp.Key{NetAddr: "1.1.1.1"})
	if !ok {
		t.Fatalf("failed to lookup arp table entry")
	}
	if entry1.NetAddr != "1.1.1.1" && entry1.HwAddr != "11-11-11-11-11-11" {
	...
	entry2, ok := table.Lookup(arp.Key{NetAddr: "1.1.1.1"})
	if !ok {
		t.Fatalf("failed to lookup arp table entry")
	}
	if entry2.NetAddr != "1.1.1.1" && entry2.HwAddr != "11-11-11-11-11-12" {
		t.Fatalf("expected entry1 value is IP: 1.1.1.1, Addr: 11-11-11-11-11-12, "+
			"but got: IP: %s, Addr: %s", entry1.NetAddr, entry1.HwAddr)
	}

	// delete test
	table.Forget(arp.Key{NetAddr: "1.1.1.1"})
	_, ok = table.Lookup(arp.Key{NetAddr: "1.1.1.1"})
	if ok {
