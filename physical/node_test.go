...
func TestNetworkTopology(t *testing.T) {
	// setup node
	node1 := physical.NewNode("R0_re")
	node2 := physical.NewNode("R1_re")
	node3 := physical.NewNode("R2_re")

	// setup interface
	intf1 := physical.NewInterface("0/0", "20.1.1.1", 40000, "11-11-11-11-11-11", node1.DataSink())
	intf2 := physical.NewInterface("0/4", "40.1.1.1", 40001, "11-11-11-11-11-12", node1.DataSink())
	attachInterface(t, node1, intf1)
	attachInterface(t, node1, intf2)

	intf3 := physical.NewInterface("0/1", "20.1.1.2", 40002, "11-11-11-11-11-13", node2.DataSink())
	intf4 := physical.NewInterface("0/2", "30.1.1.1", 40003, "11-11-11-11-11-14", node2.DataSink())
	attachInterface(t, node2, intf3)
	attachInterface(t, node2, intf4)

	intf5 := physical.NewInterface("0/3", "30.1.1.2", 40004, "11-11-11-11-11-15", node3.DataSink())
	intf6 := physical.NewInterface("0/5", "40.1.1.2", 40005, "11-11-11-11-11-16", node3.DataSink())
	attachInterface(t, node3, intf5)
	attachInterface(t, node3, intf6)

	// setup link
	link1 := physical.NewLink("Link1", 1)
	attachLink(t, intf1, link1)
	attachLink(t, intf3, link1)

	link2 := physical.NewLink("Link2", 2)
	attachLink(t, intf4, link2)
	attachLink(t, intf5, link2)

	link3 := physical.NewLink("Link3", 3)
...
func TestNodeSendReceive(t *testing.T) {
	...
	sender := physical.NewNode("R0_re")
	receiver := physical.NewNode("R1_re")
	defer func() {
		receiver.Shutdown()
		sender.Shutdown()
	}()

	intf1 := physical.NewInterface("0/0", "20.1.1.1", 40000, "11-11-11-11-11-11", sender.DataSink())
	attachInterface(t, sender, intf1)

	intf2 := physical.NewInterface("0/1", "20.1.1.2", 40002, "11-11-11-11-11-13", receiver.DataSink())
...
func attachInterface(t *testing.T, node *physical.Node, intf *physical.Interface) {
	if err := node.AttachInterface(intf); err != nil {
		t.Fatalf("failed to attach interface: %v", err)
	}
}

func attachLink(t *testing.T, intf *physical.Interface, link *physical.Link) {
	if err := intf.AttachLink(link); err != nil {
		t.Fatalf("failed to attach link: %v", err)
	}
}

func testNode(t *testing.T, node *physical.Node, intfId, linkId string, cost uint) {
	intf1_, err := node.GetInterfaceById(physical.IdOf(intfId))
	if err != nil {
	...
	if link1_.GetId() != physical.IdOf(linkId) {
